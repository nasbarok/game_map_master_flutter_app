...
class GameStateService extends ChangeNotifier {
  ...
  void setActiveGameSession(GameSession? session) {
    logger.d(
        '[GameStateService] 🎮 setActiveGameSession appelé : ID=${session?.id}');
    if (session?.gameMap != null) {
      logger.d(
          '[GameStateService] 🗺️ GameMap ID=${session!.gameMap!.id}, name=${session.gameMap!.name}');
      logger.d(
          '[GameStateService] 📐 backgroundBoundsJson: ${session.gameMap!.backgroundBoundsJson}');
      logger.d(
          '[GameStateService] 📡 satelliteBoundsJson: ${session.gameMap!.satelliteBoundsJson}');
    }
...
// 🔐 Config temporaire pour BombOperation avant création de GameSession
  ...
  void selectMap(GameMap? map) {
    logger.d(
        '[GameStateService] 🗺️ [selectMap] Carte sélectionnée : ${map?.name}');
    _selectedMap = map;
    notifyListeners();
  }
  ...
  Future<void> handleTerrainOpen(Field field, ApiService apiService) async {
    try {
      if (field.active == false) {
        logger.d(
            '[GameStateService] ℹ️ [toggleTerrainOpen] Terrain fermé : ${field.name}');
        return;
      }
      _isTerrainOpen = true;
      ...
      if (_isTerrainOpen) {
        logger.d(
            '[GameStateService] 📡 [OPEN] Terrain en cours d’ouverture : ${field.name}');
        ...
        logger.d(
            '[GameStateService] 👥 [OPEN] Appel GET /fields/${field.id}/players');
        ...
        if (players is List) {
          ...
          logger.d(
              '[GameStateService] ✅ [OPEN] Joueurs connectés restaurés ($_connectedPlayers)');
        }

        // Charger statut de jeu
        logger.d(
            '[GameStateService] 🎮 [OPEN] Appel GET /games/${field.id}/status');
  ...
  void setSelectedScenarios(List<dynamic> scenarios) {
    logger.d(
        '[GameStateService] 📜 Scénarios sélectionnés : ${scenarios.length}');
  ...
  void updateConnectedPlayers(int count) {
    logger.d(
        '[GameStateService] 👥 Nombre de joueurs connectés mis à jour : $count');
  ...
  void _startGameTimer() {
    ...
    _gameTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (_timeLeftDisplay != newDisplay) {
        ...
        logger.d(
            '[GameStateService] ⏳ Temps restant mis à jour : $_timeLeftDisplay');
        notifyListeners();
      }
    });
  }
  ...
  void addConnectedPlayer(Map<String, dynamic> player) {
    ...
    if (existingIndex == -1) {
      ...
      _connectedPlayers = _connectedPlayersList.length;
      logger.d(
          '[GameStateService] ✅ Joueur ajouté. Total connectés : $_connectedPlayers');
      notifyListeners();
    } else {
  ...
  void reset() {
    logger.d(
        '[GameStateService] 🔄 Réinitialisation de l\'état du systeme du jeu');
    _isTerrainOpen = false;
    _selectedMap = null;
    _selectedScenarios = [];
    _gameDuration = null;
  ...
  Future<void> restoreSessionIfNeeded(
    ...
    try {
      ...
      if (expectedFieldId != null) {
        ...
        activeFieldResponse = await apiService.get('fields/active/current');
        logger.d(
            '[GameStateService] 📦 [RESTORE] Réponse terrain actif : $activeFieldResponse');
      }
      ...
      logger.d(
          '[GameStateService] ✅ [RESTORE] Terrain actif : ${field.name} (ID: ${field.id}');

      // Étape 2 : Carte liée
      logger.d(
          '[GameStateService] 🔎 [RESTORE] Appel GET /maps?fieldId=${field.id}');
      ...
      if (_webSocketService == null) {
        ...
        if (!_webSocketService!.isConnected) {
          logger.d(
              '[GameStateService] ⏳ [RESTORE] En attente de connexion WebSocket...');
      ...
      try {
        logger.d(
            '[GameStateService] 🔎 [RESTORE] Requête vers /game-sessions/current-session/${field.id}');
        final GameSessionService gameSessionService =
            GetIt.I<GameSessionService>();
        final gameSession =
            await gameSessionService.getCurrentSessionByFieldId(field.id!);

        if (gameSession != null && gameSession.active == true) {
          logger.d(
              '[GameStateService] 🎮 [RESTORE] Session active détectée : ID=${gameSession.id}');
          setGameRunning(true);
          ...
          logger.d(
              '[GameStateService] ℹ️ [RESTORE] Aucune session active détectée');
          setGameRunning(false);
          setActiveGameSession(null);
        }

        logger.d(
            '[GameStateService] ✅ [RESTORE] Statut récupéré : running=$_isGameRunning, sessionId=${gameSession?.id}');
      } catch (e) {
        logger.d(
            '[GameStateService] ⚠️ [RESTORE] Erreur lors de la restauration : $e');
        setGameRunning(false);
        setActiveGameSession(null);
      }
      ...
      try {
        logger.d(
            '[GameStateService] 🔎 [RESTORE] Appel GET /fields/${field.id}/scenarios');
        final scenariosResponse =
            await apiService.get('fields/${field.id}/scenarios');
        logger.d(
            '[GameStateService] 📦 [RESTORE] Réponse scénarios : $scenariosResponse');

        if (scenariosResponse == null || scenariosResponse is! List) {
          logger.d(
              '[GameStateService] ⚠️ [RESTORE] Format inattendu pour les scénarios.');
        } else {
          _selectedScenarios =
        ...
        logger.d(
            '[GameStateService] ⚠️ [RESTORE] Erreur lors de la récupération des scénarios : $e');
      }

      // Étape 4 : Joueurs connectés
      logger.d(
          '[GameStateService] 🔎 [RESTORE] Appel GET /fields/${selectedMap.field?.id}/players');
      final players =
          await apiService.get('fields/${selectedMap.field?.id}/players');
      logger.d(
          '[GameStateService] 📦 [RESTORE] Réponse joueurs connectés : $players');

      if (players == null || players is! List) {
        logger.d(
            '[GameStateService] ⚠️ [RESTORE] Format inattendu pour les joueurs connectés.');
        return;
      }
      ...
      logger.d(
          '[GameStateService] ✅ [RESTORE] Joueurs restaurés : $_connectedPlayers');

      _teamService?.loadTeams(selectedMap.id!);
      if (_teamService!.teams.isNotEmpty) {
        logger.d(
            '[GameStateService] ✅ [RESTORE] Équipes restaurées : ${_teamService?.teams.length}');
      } else {
        logger.d('[GameStateService] ⚠️ [RESTORE] Aucune équipe trouvée.');
      }
      logger.d(
          '[GameStateService] ✅ [RESTORE] Terrain ouvert : ${field.name} (ID: ${field.id})');
      notifyListeners();
    } catch (e, stack) {
  ...
  void updateConnectedPlayersList(List<Map<String, dynamic>> newList) {
    ...
    _connectedPlayers = _connectedPlayersList.length;
    logger.d(
        '[GameStateService] 🔄 Liste des joueurs connectés mise à jour : $_connectedPlayers');
    notifyListeners();
  }
  ...
  Future<void> connectHostToField() async {
    ...
    try {
      ...
      logger
          .d('[GameStateService] 🔄 Connexion automatique du host au terrain');
      ...
      logger.d(
          '[GameStateService] ❌ Erreur lors de la connexion automatique du host: $e');
    }
  }
  ...
  Future<void> loadConnectedPlayers() async {
    ...
    try {
      ...
      if (players == null || players is! List) {
        logger.d(
            '[GameStateService] ⚠️ Format inattendu pour les joueurs connectés.');
        return;
      }
      ...
      _connectedPlayers = _connectedPlayersList.length;
      logger.d(
          '[GameStateService] ✅ Joueurs connectés chargés : $_connectedPlayers');
      notifyListeners();
    } catch (e) {
      logger.d(
          '[GameStateService] ❌ Erreur lors du chargement des joueurs connectés: $e');
    }
  }
  ...
  Future<GameSession?> getCurrentGameSession() async {
    try {
      ...
      logger.e(
          '[GameStateService] Erreur lors de la récupération de la session de jeu: $e');
      return null;
    }
  }
  ...
  Future<bool> isGameActive(int scenarioId) async {
    try {
      ...
      logger.e(
          '[GameStateService] Erreur lors de la vérification de l\'état du jeu: $e');
      return false;
    }
  }
    ...
    if (gameId == 0) {
      ...
      try {
        ...
        if (fieldId == null || scenarioId == null) {
          logger.d(
              '[GameStateService] Impossible de démarrer : fieldId ou scenarioId null');
          return false;
        }

        final response =
            await apiService.post('game-sessions/$fieldId/start', {});
        if (response != null) {
      ...
      try {
        final response =
            await _apiService.post('game-sessions/$gameId/start', {});
        if (response != null) {
          return true;
        }
        return false;
  ...
  void setBombOperationConfig(BombOperationScenarioConfig config) {
    logger.d(
        '[GameStateService] 🧨 Configuration BombOperation stockée : scenarioId=${config.scenarioId}, rôles=${config.roles}');
